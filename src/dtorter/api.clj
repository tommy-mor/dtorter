...
(defn document-interceptor [typ]
  {:enter (fn [ctx]
            (let [req (ctx :request)
              ...
              (if-not (typ doc)
                (throw (ex-info (format "document id is not type %s" typ) req)))
  ...
  [
   ["/tag/:id"
    {:parameters {:path {:id string?}}
     :interceptors [(document-interceptor :tag/name)]
     :swagger {:tags ["tag"]}

     :get {:handler (fn [{:keys [resource]}] {:status 200 :body resource})}
     :put {:parameters {:body ::sp/tag}
           :handler
   ...
   ["/item/:id"
    {:parameters {:path {:id string?}}
     :interceptors [document-interceptor]
